...
for p_ind = 1:numPixels
    [act_ind, phi_pi, phi_piu,phi_piv, phi_piw]=storePixelPhiBody(p_ind, multilev,pixel, knotvectorU, knotvectorV, knotvectorW, kuMAX, kvMAX, kwMAX, pU, pV, pW, nelemU, nelemV, nobU, nobV, nobW, unobMAX, vnobMAX, wnobMAX, Em, Coeff);
    %store the active element and the corresponding phi values
    Pixel(p_ind).active_cell = act_ind;
    Pix2{p_ind,1} = phi_pi;
    Pix2{p_ind,2} = phi_piu;
    Pix2{p_ind,3} = phi_piv;
    Pix2{p_ind,4} = phi_piw;
end
end

function [act_ind, phi_pi,phi_piu, phi_piv, phi_piw]=storePixelPhiBody(p_ind, multilev,pixel, knotvectorU, knotvectorV, knotvectorW, kuMAX, kvMAX, kwMAX, pU, pV, pW, nelemU, nelemV, nobU, nobV, nobW, unobMAX, vnobMAX, wnobMAX, Em, Coeff)
...
RR1 = Der1BasisFun((uu-1),pixel(p_ind,1),pU,knotuu);
RR2 = Der1BasisFun((vv-1),pixel(p_ind,2),pV,knotvv);
RR3 = Der1BasisFun((ww-1),pixel(p_ind,3),pW,knotww);
...
phii = RR123(end:-1:1);
phii_u = RR123u(end:-1:1);
phii_v = RR123v(end:-1:1);
phii_w = RR123w(end:-1:1);

% compute phi = coeff*Nu*Nv*Nw
phi_pi = pix_coeff*phii;
phi_piu = pix_coeff*phii_u;
phi_piv = pix_coeff*phii_v;
phi_piw = pix_coeff*phii_w;
end
