function [RHS_final,RHS_image, RHS_reg, LRHS_reg, RHS_total, final_error, theta_g_new_final,Mass_matrix_final, RHS_theta_final] = compute_Integ_Domain_skin_growth_projection(Jm,img_error,Bterm1,Bterm2,Bterm3,BIGXX, BIGYY, BIGZZ, BIGMUX,BIGMUY,BIGMUZ,BIGMVX,BIGMVY,BIGMVZ,BIGMWX,BIGMWY,BIGMWZ,RHS,PHI1,PHIU1,PHIV1,PHIW1,mu, lambda, alpha, beta,beta_1, w1,w2,w3,H,Img_source, kappa, dtime,theta_g_old,theta_crit,flag_mass,Gx_ip,Gy_ip,Gz_ip)
%#codegen
...
RHS_theta = zeros(bf_ct,4);
Mass_matrix = zeros(bf_ct,bf_ct);
...
error = 0;
theta_g_new_final = zeros(ac_ct,xlen,xlen,xlen);
...
for i=1:ac_ct
    ...
    RHS3 = zeros(bf_ct,4);
    RHS4 = zeros(bf_ct,4);
    RHS_theta_1 = zeros(bf_ct,4);
    Mass_matrix_1 = zeros(bf_ct,bf_ct);
    ...
    val1_th_1 = zeros(supp_size,1);
    Mass_matrix_2 = zeros(supp_size,supp_size);
        ...
        for gg2 = 1:xlen
            for gg3 = 1:xlen
                ...
                if(img_source1(gg1,gg2,gg3)>=0.5)
                    %Fg = sqrt(theta_g_n)*eye(3,3) - (sqrt(theta_g_n)-1)*norm_surf;
                    %Fg_inv = inv(Fg);
                ...
                if(J<=0)
                    ...
                    J = det(F);
                    Je = det(Fe);
                    fprintf('Negative Jacobian at %f, %f, %f, %f\n', gxx, gyy, gzz, double(i));
                    Ce = eye(3,3);
                    Ce_inv = eye(3,3);
                    B = eye(3,3);
                    theta_g_e = 1;
                    ...
                    J = det(F);
                    Je = det(Fe);
                    norml = [Gx(gg1,gg2,gg3);Gy(gg1,gg2,gg3);Gz(gg1,gg2,gg3)];
                    val = J*sqrt(norml'*C_inv*norml);
                    area_change = val;
                    theta_g_e = area_change/theta_g_o;
                end
                ...
                if(flag_mass==1)
                    for j = 1:size(phi_i,1)
                        for ii = 1:size(phi_i,1)
                            phiii = phi_i(j,1);
                            phjjj = phi_i(ii,1);
                            valm = phiii*phjjj;
                            Mass_matrix_2(j,ii) = Mass_matrix_2(j,ii)+(w1(gg1,1).*w2(gg2,1).*w3(gg3,1)*valm.*hu.*hv.*hw);
                        end
                    end
                else
                ...
                if(img_source1(gg1,gg2,gg3)>=0.5)
                    theta_g_n = theta_g_o + dtime.*kappa.*(theta_g_e-theta_crit);
                    theta_g_new_final(i,gg1,gg2,gg3) = theta_g_n;
                else
                    theta_g_n = theta_g_o;
                    theta_g_new_final(i,gg1,gg2,gg3) = theta_g_n;
                end
                ...
                for j = 1:size(phi_ui,1)
                    ...
                    val1_th_1(j,1) = val1_th_1(j,1) + w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*theta_g_n.*(phi_i(j,1)).*hu.*hv.*hw;
                end
    ...
    if(flag_mass==1)
        Mass_matrix_1(SB,SB) = Mass_matrix_2;
        Mass_matrix = Mass_matrix + Mass_matrix_1;
    end
    ...
    RHS_theta_1(SB,1) = val1_th_1;
    RHS_theta = RHS_theta + RHS_theta_1;
end

%theta_g_new_final = theta_g_new;
RHS_final = RHS;
...
RHS_total = RHS;
final_error = error;
Mass_matrix_final = Mass_matrix;
RHS_theta_final = RHS_theta;
end