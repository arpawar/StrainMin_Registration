function [RHS_final,RHS_image, RHS_reg, LRHS_reg, RHS_total] = compute_Integ_Domain_hyper_elastic_a(Jm,Bterm1,Bterm2,Bterm3,BIGXX, BIGYY, BIGZZ, BIGMUX,BIGMUY,BIGMUZ,BIGMVX,BIGMVY,BIGMVZ,BIGMWX,BIGMWY,BIGMWZ,RHS,PHI1,PHIU1,PHIV1,PHIW1,param,w1,w2,w3,H,Img_source)
%#codegen
        ...
        for gg2 = 1:xlen
            for gg3 = 1:xlen
                ...
                for j = 1:size(phi_ui,1)
                    ...
                    val1(j,1) = val1(j,1) + (phi_i(j,1)).*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*(dTYFdx).*hu.*hv.*hw+2.*lambda_1.*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*img_source1(gg1,gg2,gg3).*sum_finalx.*hu.*hv.*hw;
                    val2(j,1) = val2(j,1) + (phi_i(j,1)).*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*(dTYFdy).*hu.*hv.*hw+2.*lambda_1.*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*img_source1(gg1,gg2,gg3).*sum_finaly.*hu.*hv.*hw;
                    val3(j,1) = val3(j,1) + (phi_i(j,1)).*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*(dTYFdz).*hu.*hv.*hw+2.*lambda_1.*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*img_source1(gg1,gg2,gg3).*sum_finalz.*hu.*hv.*hw;
                
                    val11(j,1) = val11(j,1) + (phi_i(j,1)).*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*(dTYFdx).*hu.*hv.*hw;
                    val22(j,1) = val22(j,1) + (phi_i(j,1)).*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*(dTYFdy).*hu.*hv.*hw;
                    val33(j,1) = val33(j,1) + (phi_i(j,1)).*w1(gg1,1).*w2(gg2,1).*w3(gg3,1).*(dTYFdz).*hu.*hv.*hw;
