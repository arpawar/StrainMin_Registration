...
sphere_img = zeros(100,100,100);
sphere_2x_img = zeros(100,100,100);
radius1 = 20;
radius2 = 30;
...
for multilev = 0:1:0%param.maxlevel-1
    % this is not correct 
    %[DI1X,DI1Y,DI1Z] = gradient(Img_source);
    ...
    %     figure
    %     p = patch(isosurface(Xvtk,Yvtk,Zvtk,phi_interpolated,0.5));
    %     isonormals(Xvtk,Yvtk,Zvtk,phi_interpolated,p);
    %     p.FaceColor = 'red';
    %     p.EdgeColor = 'none';
    %     daspect([1 1 1])
    %     view(3);
    ...
    %vtkwrite('source_bspline.vtk', 'structured_grid',Xvtk,Yvtk,Zvtk,'scalars','Intensity',phi_interpolated);
    ...
    while (iterct_level < itermax)
        ...
        [RHS, RHS_image, RHS_reg, LRHS_reg, RHS_total] = compute_Integ_Domain_hyper_elastic_a(Jm,Bterm1,Bterm2,Bterm3,BIGXX,BIGYY,BIGZZ,BIGMUX,BIGMUY,BIGMUZ,BIGMVX,BIGMVY,BIGMVZ,BIGMWX,BIGMWY,BIGMWZ,RHS,PHI1,PHIU1,PHIV1,PHIW1,param,Wu,Wv,Ww,H,cII_SX);
        ...
        disp(sprintf('Iteration = %d, residual = %f',iterct_level, final_residual));
        rs(iterct,1) = RS_final;
        iterations(iterct,1) = iterct;
        
        filename_vtk = sprintf('evolve_sphere_%d.vtk',iterct);
