...
for i =1:Nx
    for j=1:Ny
        for k=1:Nz
            if(i>=36 && i<=65 && j>=36 && j<=65 && k>=36 && k<=45)
                smallplate_img(i,j,k) = 1;
            end
            if(i>=34 && i<=67 && j>=34 && j<=67 && k>=36 && k<=45)
...
for multilev = 0:1:0%param.maxlevel-1
    ...
    while (iterct_level < itermax)
        ...
        [RHS, RHS_image, RHS_reg, LRHS_reg, RHS_total] = compute_Integ_Domain_hyper_elastic_1(Jm,Bterm1,Bterm2,Bterm3,BIGXX,BIGYY,BIGZZ,BIGMUX,BIGMUY,BIGMUZ,BIGMVX,BIGMVY,BIGMVZ,BIGMWX,BIGMWY,BIGMWZ,RHS,PHI1,PHIU1,PHIV1,PHIW1,param,Wu,Wv,Ww,H,cII_SX);
        ...
        filename_vtk = sprintf('evolve_biaxial_%d.vtk',iterct);
