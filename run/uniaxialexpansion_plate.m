...
param = setparameters_uniaxial_local();
...
smallplate_img = zeros(50,50,50);
bigplate_img = zeros(50,50,50);
...
for i =1:Nx
    for j=1:Ny
        for k=1:Nz
            if(i>=15 && i<=35 && j>=13 && j<=37 && k>=20 && k<=30)
                smallplate_img(i,j,k) = 1;
            end
            if(i>=13 && i<=37 && j>=13 && j<=37 && k>=20 && k<=30)
...
Img_target = bigplate_img;
Img_source = smallplate_img;
...
vtkwrite('post_processing/source_uniaxial.vtk', 'structured_grid',Xvtk,Yvtk,Zvtk,'scalars','Intensity',Img_source);
vtkwrite('post_processing/target_uniaxial.vtk', 'structured_grid',Xvtk,Yvtk,Zvtk,'scalars','Intensity',Img_target);
...
for multilev = 0:1:param.maxlevel-1
    ...
    filename = sprintf('post_processing/Uniaxial_mesh_%d.vtk',multilev+1);
    ...
    while (iterct_level < itermax)
        ...
        save('post_processing/uniaxial_residuals.mat','iter_reg','iter_img','iterations');
        
        if(mod(iterct_level,500)==0)
            filename_vtk = sprintf('post_processing/evolve_uniaxial_local_%d.vtk',iterct);
    ...
    filename_vtk1 = sprintf('post_processing/uniaxial_deformed_%d.vtk',multilev+1);
