function [phi_interpolated_final,BIGXX,BIGYY,BIGZZ,BIGMUX,BIGMUY,BIGMUZ,BIGMVX,BIGMVY,BIGMVZ,BIGMWX,BIGMWY,BIGMWZ] = computenewPoints_phi(Jm,ACP,PHI1,PHIU1,PHIV1,PHIW1,xlen)
%#codegen
...
phi_interpolated = zeros(ac_ct,xlen,xlen,xlen);
...
for i = 1:ac_ct
    ...
    for ii =1:xlen
        for jj =1:xlen
            for kk = 1:xlen
                for kk1 = 1:size(supp_hi,1)
                    BB = Dm(1,1).actB;
                    BB_active = BB(SB(kk1,1),1);
                    f = f + source_cp(BB_active,1)*supp_phi(k,ii,jj,kk);
                end
                phi_interpolated(i,ii,jj,kk) = f;
            end
        end
    end
...
phi_interpolated_final = reshape(phi_interpolated,ac_ct*xlen,xlen,xlen);
